...
async function fetchAndProcessData() {
  ...
  const formattedDisplayHour = displayHour.toString().padStart(2, "0");
  const formattedHour = `${formattedDisplayHour}:00 ${period}`;
  const refPath = `Game1/${formattedDate}_${formattedHour}`;
  ...
  if (hour < 12 || hour > 23) {
    console.log("Time slot is outside 12 PM to 11 PM IST. No game data to process.");
    return;
  }
  ...
  try {
    ...
    // Calculate the total game collection first
    let totalAmount = 0;
    Object.values(gameData).forEach((player) => {
      if (player.selectedCards) {
        player.selectedCards.forEach((cardData) => {
    ...
    // Create a map to store the total amount for each card
    const cardTotalAmountMap = new Map();
    Object.values(gameData).forEach((player) => {
      if (player.selectedCards) {
        player.selectedCards.forEach((cardData) => {
          const { cardId, amount } = cardData;
          if (cardTotalAmountMap.has(cardId)) {
            cardTotalAmountMap.set(cardId, cardTotalAmountMap.get(cardId) + amount);
          } else {
            cardTotalAmountMap.set(cardId, amount);
          }
        });
    ...
    let bestCard = null;
    let bestLeftAmount = totalAmount;

    for (const [cardId, amount] of cardTotalAmountMap) {
      const amountSpent = amount * 12;
      const leftAmount = totalAmount - amountSpent;
      if (leftAmount < totalAmount * 0.3 && leftAmount < bestLeftAmount && leftAmount >= 0) {
    ...
    if (!bestCard) {
      const allCards = Array.from({ length: 12 }, (_, i) => `c${i + 1}`);
      const selectedCards = Array.from(cardTotalAmountMap.keys());
      const unselectedCards = allCards.filter((card) => !selectedCards.includes(card));

      if (unselectedCards.length > 0) {
        const randomLeastSelectedCard = unselectedCards[Math.floor(Math.random() * unselectedCards.length)];
        console.log("Randomly selected unselected card:", randomLeastSelectedCard);
        await set(
          ref(database, `result_game1/${formattedDate}_${formattedHour}`),
          {
            cardWon: randomLeastSelectedCard,
    ...
    for (const player of Object.values(gameData)) {
      ...
      if (player.selectedCards) {
        for (const cardData of player.selectedCards) {
          if (cardData.cardId === bestCard) {
            let winAmount = cardData.amount * 12;
            const userRef = ref(database, `username/${mobile}`);
            const userSnapshot = await get(userRef);
            const userData = userSnapshot.val();
            if (userData) {
    ...
    let leftAmount = totalAmount - amountSpent;
    const resultRef = ref(database, `result_game1/${formattedDate}_${formattedHour}`);
    await set(resultRef, {
      cardWon: bestCard,
      totalGameCollection: totalAmount,
      amountToSpent: amountSpent,
